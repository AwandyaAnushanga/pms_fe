...
export const Maintenance: React.FC<MaintenanceProps> = ({ onNavigate }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    priority: 'Medium',
    assignedTo: '',
    status: 'Not Completed', // Changed to string
    cost: ''
  });
  ...
  const fetchRequests = async () => {
    try {
      ...
      const requestsData = data?.content && Array.isArray(data.content) ? data.content : [];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingRequest) {
        const { error } = await api.maintenance.update(editingRequest.id, payload);

        if (error) throw new Error(error);
        showToast('Maintenance request updated successfully', 'success');
      } else {
        const { error } = await api.maintenance.create(payload);
  ...
  const handleEdit = (request: MaintenanceRequest) => {
    setEditingRequest(request);
    setFormData({
      unitId: (request as any).unitId || request.unit_id || '',
      issueType: (request as any).issueType || request.issue_type || '',
      description: request.description || '',
      priority: request.priority || 'Medium',
      assignedTo: (request as any).assignedTo || request.assigned_to || '',
  ...
  const filteredBySearch = (statusRequests: MaintenanceRequest[]) => {
    ...
    return statusRequests.filter(
      (req) => {
        ...
        const issueType = (req as any).issueType || req.issue_type || '';
        const unitNumber = req.unit?.unitNumber || (req.unit as any)?.unit_number || '';
  ...
  const notCompletedRequests = filteredBySearch(filterByStatus('Not Completed'));
  const startedRequests = filteredBySearch(filterByStatus('Started'));
  ...
  const KanbanColumn = ({
    ...
    <div className="flex-1 bg-gray-100 rounded-lg p-4">
      ...
      <div className="space-y-3">
        {requests.map((request) => {
          const issueType = (request as any).issueType || request.issue_type || '';
          const unitNumber = request.unit?.unitNumber || (request.unit as any)?.unit_number || 'N/A';
          const propertyName = request.unit?.property?.propertyName || (request.unit as any)?.property?.property_name || 'N/A';
          const assignedTo = (request as any).assignedTo || request.assigned_to || '';
          const createdAt = (request as any).createdAt || request.created_at;
          ...
          return (
            ...
            >
              ...
              <div className="text-xs text-gray-600 mt-2">
                <div>Created: {createdAt ? new Date(createdAt).toLocaleDateString() : 'N/A'}</div>
                <div className="font-semibold text-gray-900 mt-1">
                  Cost: ${request.cost ? Number(request.cost).toFixed(2) : '0.00'}
                </div>
              </div>
                ...
                {status !== 'Not Completed' && (
                  <Button
                    ...
                    onClick={() =>
                      handleStatusChange(
                        request.id,
                        status === 'Started' ? 'Not Completed' : 'Started'
                      )
                    }
                    className="text-xs"
                  >
                ...
                {status !== 'Completed' && (
                  <Button
                    ...
                    onClick={() =>
                      handleStatusChange(
                        request.id,
                        status === 'Not Completed' ? 'Started' : 'Completed'
                      )
                    }
                    className="text-xs"
                  >
                    {status === 'Not Completed' ? 'Start →' : 'Complete →'}
                  </Button>
                )}
                <div className="flex-1"></div>
                <Button variant="ghost" size="sm" onClick={() => handleEdit(request)}>
    ...
    >
      <div className="space-y-6">
        ...
        {loading ? (
          ...
          <div className="flex gap-4 overflow-x-auto pb-4">
            <KanbanColumn title="Not Completed" requests={notCompletedRequests} status="Not Completed" />
            <KanbanColumn title="Started" requests={startedRequests} status="Started" />
