...
//     // Access the content property which contains the actual array
//     const propertyArray = res.data?.content || [];
//     console.log("Setting properties to:", propertyArray);
...
//   const obj = Object.fromEntries(filtered.map(u => [u.id, u]));
//   setFilteredUnits(obj);
// }, [searchTerm, units]);
...
//     // Access content property for paginated response
//     const list = data?.content || [];
//     setUnits(list);

//     const unitsObject = Object.fromEntries(list.map(u => [u.id, u]));
//     setFilteredUnits(unitsObject);
//   } catch (error) {
...
//               {properties.length > 0 && (
//                 <div>First property: {JSON.stringify(properties[0])}</div>
//               )}
//             </div>
...
export const Units: React.FC<UnitsProps> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    ...
    const obj = Object.fromEntries(filtered.map((u: Unit) => [u.id, u]));
    setFilteredUnits(obj);
  }, [searchTerm, units]);
  ...
  const fetchUnits = async () => {
    try {
      ...
      const enrichedUnits = list.map((unit: any) => ({
        ...unit,
        property: properties.find(p => String(p.id) === String(unit.propertyId))
      }));
      
      setUnits(enrichedUnits);

      const unitsObject = Object.fromEntries(enrichedUnits.map((u: Unit) => [u.id, u]));
      setFilteredUnits(unitsObject);
    } catch (error) {
      showToast('Failed to fetch units', 'error');
    } finally {
  ...
  const handleEdit = (unit: Unit) => {
    ...
    setFormData({
      unitNumber: unit.unitNumber,
      propertyId: unit.property?.id 
        ? String(unit.property.id) 
        : String((unit as any).propertyId || ''),
      size: unit.size.toString(),
      floor: unit.floor.toString(),
      status: unit.status,
      rent: unit.rent.toString()
